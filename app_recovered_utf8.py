...
def get_version_info():
    """Obtener informaci├│n de versi├│n desde el archivo JSON"""
    try:
...
class Alumno(db.Model):
    ...
    def get_precio_cuota(self):
        """Obtiene el precio de la cuota seg├║n el tipo"""
        # Buscar en la configuraci├│n de tipos de clase
        tipo_clase = TipoClase.query.filter_by(codigo=self.tipo_cuota, activo=True).first()
        if tipo_clase:
            return tipo_clase.precio
        
        # Fallback a precios por defecto si no est├í configurado
        precios = {
            'clase_suelta': 15.00,
...
class Pago(db.Model):
    ...
    a├▒o = db.Column(db.Integer, nullable=True)  # A├▒o para matr├¡cula
...
class Clase(db.Model):
    ...
    duracion_minutos = db.Column(db.Integer, default=75)  # Duraci├│n por defecto de la clase
...
class HorarioSemanal(db.Model):
    ...
    def __repr__(self):
        dias = ['Lunes', 'Martes', 'Mi├®rcoles', 'Jueves', 'Viernes', 'S├íbado', 'Domingo']
...
class Tarifa(db.Model):
    ...
    def __repr__(self):
        return f'<Tarifa {self.nombre}: Ôé¼{self.precio}>'
...
# MODELOS PARA FACTURACI├ôN (Facturas emitidas a clientes)
...
class Cliente(db.Model):
    ...
    pais = db.Column(db.String(100), default='Espa├▒a')
...
# Modelo de Configuraci├│n Fiscal del Negocio
class ConfiguracionFiscal(db.Model):
    ...
    telefono = db.Column(db.String(20))
    email = db.Column(db.String(100))
    epigrafe_iae = db.Column(db.String(10))  # Ep├¡grafe IAE
    regimen_iva = db.Column(db.String(50), default='general')  # general, recargo_equivalencia, exento
    tipo_retencion_default = db.Column(db.Float, default=7.0)  # 7% o 15% seg├║n actividad
    exento_iva = db.Column(db.Boolean, default=True)  # Exenci├│n art. 20 Ley 37/1992
    texto_exencion_iva = db.Column(db.Text, default='Exento de IVA seg├║n art. 20.Uno.9┬║ de la Ley 37/1992')
...
class FacturaEmitida(db.Model):
    ...
    def calcular_totales(self):
        """Calcula los totales de la factura bas├índose en sus l├¡neas"""
    ...
    def generar_numero_factura(self):
        """Genera el n├║mero de factura correlativo seg├║n la serie"""
        a├▒o_factura = self.fecha_emision.year if self.fecha_emision else date.today().year

        # Buscar el ├║ltimo n├║mero de la serie en el a├▒o de la factura
        ultima_factura = FacturaEmitida.query.filter(
            FacturaEmitida.serie == self.serie,
            db.extract('year', FacturaEmitida.fecha_emision) == a├▒o_factura,
        ...
        self.numero_completo = f"{self.serie}/{a├▒o_factura}/{self.numero:04d}"
...
class LineaFactura(db.Model):
    ...
    def __repr__(self):
        return f'<LineaFactura {self.descripcion} - Ôé¼{self.subtotal}>'

    def calcular_subtotal(self):
        """Calcula el subtotal de la l├¡nea"""
        self.subtotal = self.cantidad * self.precio_unitario

# MODELOS PARA GESTI├ôN ECON├ôMICA
...
class FacturaProveedor(db.Model):
    ...
    def esta_vencida(self):
        """Verifica si la factura est├í vencida"""
...
class GastoFijo(db.Model):
    ...
    dia_cargo = db.Column(db.Integer, default=1)  # D├¡a del mes para el cargo
    activo = db.Column(db.Boolean, default=True)
    ...
    def get_importe_anual(self):
        """Calcula el importe anual seg├║n la frecuencia"""
    ...
    def __repr__(self):
        return f'<GastoFijo {self.nombre} - {self.importe}Ôé¼/{self.frecuencia}>'
...
class GastoMensual(db.Model):
    ...
    def __repr__(self):
        return f'<GastoMensual {self.concepto} - {self.importe}Ôé¼>'
...
class TipoClase(db.Model):
    ...
    descripcion = db.Column(db.Text)  # Descripci├│n detallada
    precio = db.Column(db.Float, nullable=False)
    ...
    color = db.Column(db.String(7), default='#007bff')  # Color para visualizaci├│n
    fecha_creacion = db.Column(db.DateTime, default=datetime.utcnow)
    
    def __repr__(self):
        return f'<TipoClase {self.nombre}: {self.precio}Ôé¼>'
    ...
    def get_precio_display(self):
        ...
        unidades = {
            'por_clase': 'Ôé¼/clase',
            'mensual': 'Ôé¼/mes',
            'bimensual': 'Ôé¼/bimestre',
            'anual': 'Ôé¼/a├▒o'
        }
        return f"{self.precio} {unidades.get(self.frecuencia, 'Ôé¼')}"
...
class SesionYogaterapia(db.Model):
    ...
    hora_inicio = db.Column(db.Time)  # Hora de inicio de la sesi├│n
    hora_fin = db.Column(db.Time)  # Hora de fin de la sesi├│n
    ...
    motivo_consulta = db.Column(db.Text)  # Por qu├® viene la persona
    objetivos_terapeuticos = db.Column(db.Text)  # Objetivos espec├¡ficos
    tecnicas_aplicadas = db.Column(db.Text)  # T├®cnicas de yoga utilizadas
    posturas_trabajadas = db.Column(db.Text)  # Asanas espec├¡ficas
    respiracion_pranayama = db.Column(db.Text)  # Ejercicios de respiraci├│n
    meditacion_relajacion = db.Column(db.Text)  # T├®cnicas de meditaci├│n
    
    # Evaluaci├│n y seguimiento
    estado_inicial = db.Column(db.Text)  # C├│mo lleg├│ la persona
    respuesta_sesion = db.Column(db.Text)  # C├│mo respondi├│ durante la sesi├│n
    estado_final = db.Column(db.Text)  # C├│mo se fue la persona
    observaciones_terapeuta = db.Column(db.Text)  # Observaciones profesionales
    recomendaciones_casa = db.Column(db.Text)  # Pr├íctica para casa
    proxima_sesion = db.Column(db.Text)  # Plan para pr├│xima sesi├│n
...
class ArchivoYogaterapia(db.Model):
    ...
    descripcion = db.Column(db.String(200))
    tama├▒o_bytes = db.Column(db.Integer)
...
class Sutra(db.Model):
    ...
    sanscrito = db.Column(db.Text, nullable=False)  # Texto en s├ínscrito
    transliteracion = db.Column(db.Text, nullable=False)  # Transliteraci├│n
    traduccion = db.Column(db.Text, nullable=False)  # Traducci├│n al espa├▒ol
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password_hash, password):
            ...
            flash(f'┬íBienvenido, {user.nombre}!', 'success')
...
def nuevo_alumno():
    if request.method == 'POST':
                try:
                    ...
                    db.session.commit()
                    flash('┬íAlumno registrado exitosamente!', 'success')
...
def ver_alumno(alumno_id):
    ...
    # Obtener asistencias del alumno (├║ltimos 90 d├¡as)
    ...
    # Asistencias por mes (├║ltimos 3 meses)
    asistencias_por_mes = {}
...
def editar_alumno(alumno_id):
    ...
    if request.method == 'POST':
        try:
            ...
            db.session.commit()
            flash('┬íAlumno actualizado exitosamente!', 'success')
...
def eliminar_alumno(alumno_id):
    ...
    if request.method == 'POST':
        alumno.activo = False
        db.session.commit()
        flash('┬íAlumno desactivado exitosamente!', 'success')
        return redirect(url_for('alumnos'))
    else:
        if request.args.get('confirm') == '1':
            ...
            flash('Confirma la desactivaci├│n del alumno.', 'warning')
            return redirect(url_for('alumnos'))
...
def agregar_pago(alumno_id):
    ...
    if request.method == 'POST':
        try:
            ...
            # Manejar mes y a├▒o seg├║n el tipo de pago
            mes = None
            a├▒o = None
            fecha_clase = None
            ...
            if tipo_pago == 'cuota':
                ...
                if pago_existente:
                    flash(f'ÔÜá´©Å Ya existe un pago de cuota para {mes}. Si necesitas modificarlo, edita el pago existente.', 'warning')
                    return redirect(url_for('ver_alumno', alumno_id=alumno_id))
                    
            elif tipo_pago == 'matricula':
                a├▒o = int(request.form.get('a├▒o', date.today().year))
                # Verificar si ya existe un pago de matr├¡cula para este a├▒o
                pago_existente = Pago.query.filter_by(
                    alumno_id=alumno_id,
                    tipo_pago='matricula',
                    a├▒o=a├▒o
                ...
                if pago_existente:
                    flash(f'ÔÜá´©Å Ya existe un pago de matr├¡cula para el a├▒o {a├▒o}. Si necesitas modificarlo, edita el pago existente.', 'warning')
                    return redirect(url_for('ver_alumno', alumno_id=alumno_id))
                    
            elif tipo_pago == 'clase_suelta':
                fecha_clase = datetime.strptime(request.form['fecha_clase'], '%Y-%m-%d').date()
                # Verificar si ya existe un pago para esta fecha espec├¡fica
                pago_existente = Pago.query.filter_by(
                    alumno_id=alumno_id,
                ...
                if pago_existente:
                    flash(f'ÔÜá´©Å Ya existe un pago de clase suelta para el {fecha_clase.strftime("%d/%m/%Y")}. Si necesitas modificarlo, edita el pago existente.', 'warning')
                    return redirect(url_for('ver_alumno', alumno_id=alumno_id))
            
            # Crear descripci├│n seg├║n el tipo de pago
            if tipo_pago == 'matricula':
                # Calcular formato de a├▒o acad├®mico
                if a├▒o >= 2025:
                    matricula_desc = f"Matr├¡cula {(a├▒o % 100)}/{((a├▒o + 1) % 100)}"
                else:
                    matricula_desc = f"Matr├¡cula {a├▒o}"
            
            descripciones = {
                'cuota': f'Cuota mensual - {alumno.get_tipo_cuota_display()}',
                'matricula': matricula_desc if tipo_pago == 'matricula' else f'Matr├¡cula {a├▒o}',
            ...
            # Si es matr├¡cula, marcar al alumno como que pag├│ la matr├¡cula
            if tipo_pago == 'matricula':
                alumno.matricula_pagada = True
                alumno.fecha_matricula = date.today()
            
            db.session.commit()
            flash('┬íPago registrado exitosamente!', 'success')
            return redirect(url_for('ver_alumno', alumno_id=alumno_id))
        except Exception as e:
...
def editar_pago(pago_id):
    ...
    if request.method == 'POST':
        try:
            ...
            # Actualizar campos espec├¡ficos seg├║n el tipo
            if pago.tipo_pago == 'cuota':
                nuevo_mes = request.form.get('mes')
                # Verificar duplicados solo si cambi├│ el mes
                if nuevo_mes != pago.mes:
                    ...
                    if pago_existente:
                        flash(f'ÔÜá´©Å Ya existe un pago de cuota para {nuevo_mes}', 'warning')
                        return redirect(url_for('ver_alumno', alumno_id=alumno.id))
                pago.mes = nuevo_mes
                
            elif pago.tipo_pago == 'matricula':
                nuevo_a├▒o = int(request.form.get('a├▒o', date.today().year))
                # Verificar duplicados solo si cambi├│ el a├▒o
                if nuevo_a├▒o != pago.a├▒o:
                    pago_existente = Pago.query.filter_by(
                        alumno_id=pago.alumno_id,
                        tipo_pago='matricula',
                        a├▒o=nuevo_a├▒o
                    ...
                    if pago_existente:
                        flash(f'ÔÜá´©Å Ya existe un pago de matr├¡cula para el a├▒o {nuevo_a├▒o}', 'warning')
                        return redirect(url_for('ver_alumno', alumno_id=alumno.id))
                pago.a├▒o = nuevo_a├▒o
                
            elif pago.tipo_pago == 'clase_suelta':
                nueva_fecha = datetime.strptime(request.form['fecha_clase'], '%Y-%m-%d').date()
                # Verificar duplicados solo si cambi├│ la fecha
                if nueva_fecha != pago.fecha_clase:
                    ...
                    if pago_existente:
                        flash(f'ÔÜá´©Å Ya existe un pago de clase suelta para el {nueva_fecha.strftime("%d/%m/%Y")}', 'warning')
                        return redirect(url_for('ver_alumno', alumno_id=alumno.id))
                pago.fecha_clase = nueva_fecha
            
            db.session.commit()
            flash('┬íPago actualizado exitosamente!', 'success')
            return redirect(url_for('ver_alumno', alumno_id=alumno.id))
    ...
    # Calcular a├▒o de matr├¡cula para el template
...
def eliminar_pago(pago_id):
    ...
    try:
        ...
        # Si es matr├¡cula, actualizar el estado del alumno
        if pago.tipo_pago == 'matricula':
            # Verificar si hay otros pagos de matr├¡cula
...
def horarios():
    ...
    # Calcular estad├¡sticas de periodicidad
    clases_periodicidad = {
...
def nuevo_horario():
    ...
    # Para GET, simplemente redirigir a horarios (el modal est├í en esa p├ígina)
    return redirect(url_for('horarios'))
...
def registrar_asistencia():
    if request.method == 'POST':
        try:
            ...
            db.session.commit()
            flash('┬íAsistencia registrada exitosamente!', 'success')
            return redirect(url_for('calendario'))
        except Exception as e:
...
def calendario_unificado_viejo():
    """Calendario unificado con actividades peri├│dicas y citas individuales (versi├│n original)"""
    # Obtener par├ímetros de fecha
    a├▒o = request.args.get('a├▒o', datetime.now().year, type=int)
    mes = request.args.get('mes', datetime.now().month, type=int)
    vista = request.args.get('vista', 'mes')  # mes, semana
    
    if vista == 'semana':
        return calendario_semanal(a├▒o, mes)
    
    # Obtener sesiones de yogaterapia del mes
    sesiones_yogaterapia = SesionYogaterapia.query.filter(
        db.extract('year', SesionYogaterapia.fecha_sesion) == a├▒o,
    ...
    primer_dia = datetime(a├▒o, mes, 1).weekday()
    ...
    for dia in range(1, dias_en_mes + 1):
        fecha_actual = datetime(a├▒o, mes, dia).date()
...
def calendario_semanal(a├▒o, mes):
...
def calendario_anual():
    ...
    # Obtener todas las sesiones de yogaterapia del a├▒o
    sesiones_yogaterapia = SesionYogaterapia.query.filter(
        db.extract('year', SesionYogaterapia.fecha_sesion) == a├▒o
    ).order_by(SesionYogaterapia.fecha_sesion).all()
...
def horarios_historico():
    """Vista del hist├│rico de horarios y cambios"""
    # Obtener par├ímetros de filtro
    fecha_desde = request.args.get('fecha_desde')
    fecha_hasta = request.args.get('fecha_hasta')
    clase_id = request.args.get('clase_id', type=int)
    
    # Consulta base de horarios (incluyendo inactivos para ver hist├│rico)
...
def asistencias_historico():
    """Vista del hist├│rico de asistencias por clase"""
    # Obtener par├ímetros de filtro
    fecha_desde = request.args.get('fecha_desde')
    fecha_hasta = request.args.get('fecha_hasta')
    clase_id = request.args.get('clase_id', type=int)
...
@login_required
def yogaterapia():
    """P├ígina principal de yogaterapia (sesiones de yogaterapia)"""
...
def nueva_yogaterapia():
    """Formulario para nueva sesi├│n de yogaterapia"""
    alumnos = Alumno.query.filter_by(activo=True).order_by(Alumno.nombre, Alumno.apellido).all()
...
def procesar_yogaterapia_general():
    """Procesar nueva sesi├│n de yogaterapia general"""
    try:
        ...
        if hora_inicio and hora_fin:
            ...
            if conflicto:
                flash(f'ÔÜá´©Å {info_conflicto["mensaje"]}', 'warning')
        ...
        for archivo in archivos:
            if archivo and archivo.filename:
                ...
                archivo_db = ArchivoYogaterapia(
                    ...
                    tama├▒o_bytes=os.path.getsize(filepath) if os.path.exists(filepath) else 0
                )
                db.session.add(archivo_db)
        
        # Si est├í pagado, crear registro de pago
        if sesion.pagado:
            pago = Pago(
                alumno_id=alumno_id,
        ...
        db.session.commit()
        flash(f'┬íSesi├│n de yogaterapia registrada exitosamente para {alumno.nombre} {alumno.apellido}!', 'success')
        return redirect(url_for('yogaterapia'))
        
    except Exception as e:
        flash(f'Error al registrar sesi├│n: {str(e)}', 'error')
        db.session.rollback()
        return redirect(url_for('nueva_yogaterapia'))
...
def ver_sesion_yogaterapia(sesion_id):
    """Ver detalles de una sesi├│n de yogaterapia"""
...
def editar_sesion_yogaterapia(sesion_id):
    """Editar una sesi├│n de yogaterapia"""
    sesion = SesionYogaterapia.query.get_or_404(sesion_id)
    ...
    if request.method == 'POST':
        try:
            ...
            if hora_inicio_nueva and hora_fin_nueva:
                ...
                if conflicto:
                    flash(f'ÔÜá´©Å Conflicto de horario detectado! Ya existe una cita de {sesion_conflicto.hora_inicio.strftime("%H:%M")} a {sesion_conflicto.hora_fin.strftime("%H:%M")} con {sesion_conflicto.alumno.nombre} {sesion_conflicto.alumno.apellido}', 'warning')
            ...
            db.session.commit()
            flash('┬íSesi├│n de yogaterapia actualizada exitosamente!', 'success')
            return redirect(url_for('ver_sesion_yogaterapia', sesion_id=sesion_id))
            
        except Exception as e:
            flash(f'Error al actualizar sesi├│n: {str(e)}', 'error')
            db.session.rollback()
...
def obtener_citas_fecha(fecha):
    """API para obtener citas de una fecha espec├¡fica (para el calendario)"""
    try:
        ...
        # Obtener horarios semanales para ese d├¡a
        ...
        return jsonify({'error': 'Formato de fecha inv├ílido'}), 400
...
def agregar_archivo_yogaterapia(sesion_id):
    """Agregar archivos a una sesi├│n de yogaterapia"""
    try:
        sesion = SesionYogaterapia.query.get_or_404(sesion_id)
...
def eliminar_sesion_yogaterapia(sesion_id):
    """Eliminar una sesi├│n de yogaterapia"""
    sesion = SesionYogaterapia.query.get_or_404(sesion_id)
    ...
    try:
        ...
        flash(f'Ô£à Sesi├│n de {sesion.alumno.nombre} {sesion.alumno.apellido} eliminada exitosamente', 'success')
        return redirect(url_for('yogaterapia'))
        
    except Exception as e:
        flash(f'Error al eliminar sesi├│n: {str(e)}', 'error')
        db.session.rollback()
        return redirect(url_for('yogaterapia'))
...
def marcar_sesion_pagada(sesion_id):
    ...
    try:
        ...
        # Para yogaterapia no creamos registro de pago autom├íticamente
        # ya que no est├í asociado a un alumno espec├¡fico
        
        db.session.commit()
        flash('┬íSesi├│n marcada como pagada!', 'success')
...
# RUTAS PARA GESTI├ôN ECON├ôMICA

@app.route('/economia')
@login_required
def economia():
    """Dashboard principal de contabilidad con per├¡odos"""
    # Obtener par├ímetros de per├¡odo
    periodo = request.args.get('periodo', 'mes_actual')
    a├▒o = int(request.args.get('a├▒o', date.today().year))
    ...
    if periodo == 'mes_actual':
        ...
        if trimestre == 4:
            ...
            fecha_fin = date(a├▒o, mes_inicio + 3, 1) - timedelta(days=1)
    elif periodo == 'a├▒o':
        fecha_inicio = date(a├▒o, 1, 1)
        fecha_fin = date(a├▒o, 12, 31)
    else:  # mes_actual por defecto
        fecha_inicio = date(a├▒o, mes, 1)
        fecha_fin = date(a├▒o, mes + 1, 1) - timedelta(days=1) if mes < 12 else date(a├▒o, 12, 31)
    ...
    gastos_pendientes = 0  # Por ahora 0, se puede implementar l├│gica m├ís compleja
...
# RUTAS DE EXPORTACI├ôN Y REPORTES
...
def exportar_datos(tipo):
    ...
    try:
        if tipo == 'facturas':
            ...
            flash('Tipo de exportaci├│n no v├ílido', 'error')
...
def guardar_configuracion():
    try:
        ...
        configuraciones = [
            ...
            ('precio_matricula', request.form.get('precio_matricula', '25.00'), 'Precio de matr├¡cula anual'),
            ...
            ('direccion_escuela', request.form.get('direccion_escuela', ''), 'Direcci├│n de la escuela'),
            ('telefono_escuela', request.form.get('telefono_escuela', ''), 'Tel├®fono de contacto'),
            ('email_escuela', request.form.get('email_escuela', ''), 'Email de contacto'),
            ('web_escuela', request.form.get('web_escuela', 'http://atmasuddhi.es'), 'P├ígina web'),
            ...
            ('telefono_instructora', request.form.get('telefono_instructora', ''), 'Tel├®fono de la instructora'),
            
            # Configuraci├│n de pagos
            ('numero_cuenta', request.form.get('numero_cuenta', ''), 'N├║mero de cuenta bancaria'),
        ...
        db.session.commit()
        flash('┬íConfiguraci├│n guardada exitosamente!', 'success')
...
# RUTAS DE GESTI├ôN DE TIPOS DE CLASE

@app.route('/configuracion/tipos-clase')
@login_required
def tipos_clase():
    """Vista de gesti├│n de tipos de clase"""
...
def nuevo_tipo_clase():
    ...
    if request.method == 'POST':
        try:
            ...
            db.session.add(tipo)
            db.session.commit()
            flash('┬íTipo de clase creado exitosamente!', 'success')
...
def editar_tipo_clase(tipo_id):
    ...
    if request.method == 'POST':
        try:
            ...
            db.session.commit()
            flash('┬íTipo de clase actualizado exitosamente!', 'success')
            return redirect(url_for('tipos_clase'))
        except Exception as e:
...
# RUTAS DE BACKUP Y EXPORTACI├ôN

@app.route('/backup')
@login_required
def backup():
    """P├ígina de gesti├│n de backups"""
...
def restaurar_backup():
    ...
    try:
        ...
        if file and file.filename.endswith('.db'):
            ...
            flash('Backup restaurado exitosamente. Se cre├│ un backup del estado anterior.', 'success')
        else:
            flash('Formato de archivo no v├ílido. Solo se permiten archivos .db', 'error')
...
@login_required
def exportar():
    """P├ígina principal de exportaci├│n"""
    return render_template('exportar.html')

# Funci├│n para inicializar las clases b├ísicas
# FUNCIONES AUXILIARES
def verificar_conflicto_horario(fecha, hora_inicio, hora_fin, sesion_id=None):
    """Verificar si hay conflicto de horarios en una fecha espec├¡fica"""
    ...
    # Si estamos editando, excluir la sesi├│n actual
    if sesion_id:
...
def obtener_proximas_citas(limite=5):
    """Obtener las pr├│ximas citas individuales para el dashboard"""
    hoy = datetime.now().date()
...
def inicializar_clases():
    clases_basicas = [
        ...
        {'nombre': 'Yoga integral', 'descripcion': 'Pr├íctica completa de yoga que integra posturas, respiraci├│n y meditaci├│n', 'precio': 15.00, 'duracion_minutos': 60},
        {'nombre': 'Yoga embarazadas', 'descripcion': 'Yoga adaptado para mujeres embarazadas', 'precio': 15.00, 'duracion_minutos': 60},
        {'nombre': 'Meditaci├│n', 'descripcion': 'Pr├íctica de meditaci├│n y mindfulness', 'precio': 12.00, 'duracion_minutos': 45},
        {'nombre': 'Yogaterapia', 'descripcion': 'Sesi├│n individual de yogaterapia personalizada', 'precio': 45.00, 'duracion_minutos': 60}
    ]
    ...
    try:
        db.session.commit()
        print("Ô£à Clases b├ísicas inicializadas")
    except Exception as e:
        db.session.rollback()
        print(f"Error al inicializar clases: {e}")

def inicializar_categorias_gastos():
    """Inicializar categor├¡as de gastos predeterminadas"""
    categorias_default = [
        ...
        {'nombre': 'Marketing', 'descripcion': 'Publicidad y promoci├│n', 'color': '#007bff'},
        {'nombre': 'Formaci├│n', 'descripcion': 'Cursos y certificaciones', 'color': '#6f42c1'},
    ...
    try:
        ...
        db.session.commit()
        print("Ô£à Categor├¡as de gastos inicializadas")
    except Exception as e:
        db.session.rollback()
        print(f"Error al inicializar categor├¡as: {e}")
...
def guardar_configuracion():
    try:
        # TODO: Implementar l├│gica de backup real
        import shutil
        import os
        from datetime import datetime
...
def nueva_clase():
    ...
    if request.method == 'POST':
        try:
            ...
            db.session.add(clase)
            db.session.commit()
            flash('┬íClase creada exitosamente!', 'success')
            return redirect(url_for('configuracion'))
        except Exception as e:
...
def editar_clase(clase_id):
    ...
    if request.method == 'POST':
        try:
            ...
            db.session.commit()
            flash('┬íClase actualizada exitosamente!', 'success')
            return redirect(url_for('configuracion'))
        except Exception as e:
...
# RUTAS PARA GESTI├ôN DE USUARIOS
    ...
    if request.method == 'POST':
        try:
            ...
            if Usuario.query.filter_by(email=request.form['email']).first():
                flash('El email ya est├í registrado', 'error')
                return render_template('nuevo_usuario.html')
            
            # Crear hash de la contrase├▒a (simple para demo)
            import hashlib
            ...
            db.session.commit()
            flash('┬íUsuario creado exitosamente!', 'success')
...
def editar_usuario(usuario_id):
    ...
    if request.method == 'POST':
        try:
            ...
            db.session.commit()
            flash('┬íUsuario actualizado exitosamente!', 'success')
...
# Ruta para servir archivos est├íticos de uploads
@app.route('/uploads/<path:filename>')
def uploaded_file(filename):
    """Servir archivos subidos desde la carpeta uploads"""
    return send_from_directory(app.config['UPLOAD_FOLDER'], filename)

# Ruta para calendario de selecci├│n de citas
...
def exportar_facturas_excel(facturas):
    ...
    try:
        ...
        for factura in facturas:
            data.append({
                'N├║mero Factura': factura.numero_factura,
                'Proveedor': factura.proveedor.nombre,
                'Categor├¡a': factura.categoria.nombre,
                ...
                'M├®todo Pago': factura.metodo_pago or '',
                'Descripci├│n': factura.descripcion or '',
        ...
        flash('Error: pandas y openpyxl no est├ín instalados. Instale con: pip install pandas openpyxl', 'error')
        return redirect(url_for('facturas'))
    except Exception as e:
    ...
    try:
        ...
        for proveedor in proveedores:
            data.append({
                ...
                'Direcci├│n': proveedor.direccion or '',
                'Tel├®fono': proveedor.telefono or '',
                'Email': proveedor.email or '',
                'Contacto Principal': proveedor.contacto_principal or '',
                'Activo': 'S├¡' if proveedor.activo else 'No',
    ...
    try:
        ...
        for pago in pagos:
            data.append({
                ...
                'Mes/A├▒o': pago.mes or f"{pago.a├▒o}" if pago.a├▒o else '',
                'Fecha Clase': pago.fecha_clase.strftime('%d/%m/%Y') if pago.fecha_clase else '',
                'Monto': pago.monto,
                'M├®todo Pago': pago.metodo_pago or '',
                'Descripci├│n': pago.descripcion or ''
            })
...
def generar_reporte_pdf(facturas, gastos_fijos, ingresos, fecha_inicio, fecha_fin):
    ...
    try:
        ...
        story.append(Paragraph(f"Per├¡odo: {fecha_inicio.strftime('%d/%m/%Y')} - {fecha_fin.strftime('%d/%m/%Y')}", styles['Normal']))
        ...
        # Facturas
        if facturas:
            story.append(Paragraph("FACTURAS DEL PER├ìODO", styles['Heading2']))
            facturas_data = [['N├║mero', 'Proveedor', 'Fecha', 'Importe', 'Estado']]
            for factura in facturas:
        ...
        if gastos_fijos:
            ...
            gastos_data = [['Nombre', 'Categor├¡a', 'Importe', 'Frecuencia']]
        ...
        flash('Error: reportlab no est├í instalado. Instale con: pip install reportlab', 'error')
        return redirect(url_for('economia'))
    except Exception as e:
        flash(f'Error al generar reporte PDF: {str(e)}', 'error')
        return redirect(url_for('economia'))
...
def api_eventos_semana():
    """API para obtener eventos de una semana espec├¡fica"""
    try:
        ...
        # Obtener horarios semanales que no tienen eventos espec├¡ficos
        horarios_semanales = HorarioSemanal.query.filter_by(activo=True).all()
        
        eventos_data = []
        
        # Procesar eventos espec├¡ficos del calendario
        ...
        while fecha_actual <= fin:
            ...
            for horario in horarios_dia:
                # Verificar si ya existe un evento espec├¡fico para esta fecha/hora
...
def api_crear_evento():
    ...
    try:
        ...
        if data.get('tipo_clase') == 'yogaterapia':
            ...
            titulo = f"Yogaterapia - {data.get('alumno_nombre', 'Sesi├│n Individual')}"
            clase_id = None
...
# Rutas para gesti├│n de tipos de clases
...
def modo_pruebas():
    """P├ígina de gesti├│n de datos de prueba"""
    stats = {
...
def cargar_datos_prueba():
    ...
    try:
        ...
        flash('┬íDatos de prueba cargados exitosamente!', 'success')
    except Exception as e:
...
def resetear_sistema():
    """Resetea todos los datos del sistema (excepto configuraci├│n base)"""
    try:
        ...
        print("- L├¡neas de factura eliminadas")
...
def facturacion():
    ...
    total_facturado = db.session.query(db.func.sum(FacturaEmitida.base_imponible)).filter(
        db.extract('year', FacturaEmitida.fecha_emision) == a├▒o_actual,
...
def clientes_facturacion():
    """Listado de clientes para facturaci├│n"""
    ...
    if request.method == 'POST':
        try:
            ...
            cliente = Cliente(
                ...
                pais=request.form.get('pais', 'Espa├▒a'),
                email=request.form.get('email'),
                telefono=request.form.get('telefono'),
...
def editar_cliente(cliente_id):
    ...
    if request.method == 'POST':
        try:
            # Verificar NIF ├║nico (excepto el actual)
            nif_cif = request.form.get('nif_cif', '').strip()
            ...
            cliente.pais = request.form.get('pais', 'Espa├▒a')
    ...
    if request.method == 'POST':
        try:
            ...
            factura = FacturaEmitida(
                ...
                observaciones=request.form.get('observaciones'),
                base_imponible=0,  # Se calcular├í despu├®s
                total=0  # Se calcular├í despu├®s
            )

            # Asignar motivo de exenci├│n si aplica
            ...
            for i in range(lineas_count):
                ...
                if descripcion:  # Solo agregar l├¡neas con descripci├│n
                    linea = LineaFactura(
...
def anular_factura_emitida(factura_id):
    ...
    try:
        ...
        if factura.estado == 'anulada':
            flash('La factura ya est├í anulada', 'warning')
...
def configuracion_fiscal():
    ...
    if request.method == 'POST':
        try:
            ...
            flash('Configuraci├│n fiscal actualizada exitosamente', 'success')
            return redirect(url_for('facturacion'))
        except Exception as e:
            flash(f'Error al actualizar la configuraci├│n fiscal: {str(e)}', 'error')
            db.session.rollback()
...
@app.route('/economia')
def economia():
    """Dashboard principal de gesti├│n econ├│mica unificado"""
    # Resumen financiero del mes y a├▒o actual
    current_month = date.today().month
    current_year = date.today().year

    # INGRESOS - Facturas emitidas a clientes
    facturas_emitidas_a├▒o = FacturaEmitida.query.filter(
        db.extract('year', FacturaEmitida.fecha_emision) == current_year,
        FacturaEmitida.estado != 'anulada'
    ).all()

    total_facturado_a├▒o = sum(f.base_imponible for f in facturas_emitidas_a├▒o)
    ...
    return render_template('economia/dashboard.html',
                         # Facturas emitidas
                         total_facturado_a├▒o=total_facturado_a├▒o,
...
def api_eventos_disponibles():
    ...
    try:
        ...
        if not all([alumno_id, fecha_inicio, fecha_fin]):
            return jsonify({'success': False, 'message': 'Par├ímetros incompletos'}), 400
        ...
        for evento in eventos_query:
            # Verificar si el alumno ya est├í inscrito
            ...
            db.session.add(factura)
            db.session.commit()
            flash('┬íFactura registrada exitosamente!', 'success')
            return redirect(url_for('facturas'))
        except Exception as e:
...
def nuevo_gasto_fijo():
    ...
    if request.method == 'POST':
        try:
            ...
            db.session.add(gasto)
            db.session.commit()
            flash('┬íGasto fijo creado exitosamente!', 'success')
            return redirect(url_for('gastos_fijos'))
        except Exception as e:
            flash(f'Error al crear gasto fijo: {str(e)}', 'error')
            db.session.rollback()
...
def export_data(tipo):
    """Exportar datos econ├│micos a CSV"""
    import csv
    ...
    if tipo == 'facturas':
        ...
        writer.writerow(['N├║mero', 'Proveedor', 'Categor├¡a', 'Fecha', 'Importe Sin IVA', 'IVA', 'Total', 'Estado', 'Descripci├│n'])
        ...
        writer = csv.writer(output)
        writer.writerow(['Nombre', 'Categor├¡a', 'Importe', 'Frecuencia', 'D├¡a Cargo', 'Fecha Inicio', 'Fecha Fin', 'Activo'])
        ...
        for gasto in gastos:
            writer.writerow([
                ...
                'S├¡' if gasto.activo else 'No'
            ])
    
    elif tipo == 'ingresos':
        ingresos = Ingreso.query.all()
        writer = csv.writer(output)
        writer.writerow(['Concepto', 'Importe', 'Fecha', 'Tipo', 'M├®todo Pago', 'Alumno', 'Notas'])
        ...
        # Tambi├®n incluir eventos virtuales de horarios semanales
        horarios_semanales = HorarioSemanal.query.filter_by(activo=True).all()
...
def debug_calendario():
    """P├ígina de debug para verificar el calendario"""
    return render_template('debug_calendario.html')

@app.route('/calendario-simple')
def calendario_simple():
    """Versi├│n simplificada del calendario para debugging"""
...
def get_eventos_calendario():
    ...
    if start and end:
        ...
        # Tambi├®n soporta: 2025-12-29T00:00:00Z, 2025-12-29T00:00:00

        # Quitar zona horaria (puede venir como ' 01:00', '+01:00', 'Z' o nada)
        # Buscamos el ├║ltimo 'T' y tomamos todo hasta el ├║ltimo ':'
    ...
    while current_date <= end_date.date():
        ...
        for horario in horarios:
            if horario.dia_semana == dia_semana:
                ...
                # Construir t├¡tulo con informaci├│n de asistencia
...
def crear_evento_calendario():
    ...
    try:
        ...
        # Generar t├¡tulo autom├ítico si no se proporciona
