...
- **Migrations**: Flask-Migrate is wired in `app.py` (`migrations/` holds the baseline). New schema change = `flask db migrate -m "..."` + review + `flask db upgrade`. Existing installs that predate migrations must run `FLASK_APP=app.py flask db stamp head` once. `db.create_all()` still runs for fresh installs via `init_db.py`.
